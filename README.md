...
class adsorbate_constructor():
	...
	def __init__(self,ads_species,bond_dist,site_species=None,site_idx=None,
		...
		"""
```
The only required arguments are `ads_species`, `bond_dist`, and either `site_species` or `site_idx`. Once the `adsorbate_constructor` is instanced, one of three routines can be called: `get_adsorbate_raspa`, `get_adsorbate_pm`, and `get_adsorbate_zeo_oms`.

Molecular Adsorbates
-----
The `get_adsorbate_raspa` function is used to add a molecular adsorbate (currently supports CH4) to the adsorption site of a MOF based on a molecular mechanics energy grid generated by the molecular simulation program RASPA, and the molecular adsorbate is initialized in the lowest energy position within some cutoff distance from the proposed adsorption site. The `get_adsorbate_raspa` function is described below:
...
The script below taken from `examples/add_CH4.py` reads in CIF files from `mof_path`, adds CH4 within 3.0 A of the last O atom in the CIF file (ensuring none of the atoms in CH4 overlap within 1.3 A of the MOF), and stores the new CIF files with CH4 adsorbate in `new_mofs_path`. It assumes that the RASPA-generated energy grids are located in a folder named `examples/oxygenated_MOFs/energy_grids` since `grid_path` is not set in `get_adsorbate_raspa`. 
