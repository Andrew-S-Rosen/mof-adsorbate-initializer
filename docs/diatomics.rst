...
5. The ``d_bond`` keyword argument is the desired distance between X1 and X2 (in Å). If not specified, it will default to the value for ``bond_dist``. Here, we decided to set ``d_bond`` to 1.2 Å, which is a reasonable O-O bond distance.
