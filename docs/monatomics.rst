...
That takes care of initializing the :class:`mai.adsorbate_constructor` object. Now we can use this object to call a function to initialize the adsorbate. This is done via :func:`mai.adsorbate_constructor.get_adsorbate`. Generally, the only information you'll need to provide is the file path to the CIF of the MOF. In this case, we set it to `Cu-BTC.cif` in our current working directory. The output of calling :func:`mai.adsorbate_constructor.get_adsorbate` is a new ASE `Atoms` object with the adsorbate initialized.
